import { HttpClient, HttpHeaders, HttpParamsOptions, HttpParams, HttpErrorResponse } from '@angular/common/http';
...
export class UserfindetailsService {
  
  serbase: string = environment.URL_BASE;
  baseUrl: string = this.serbase +'/api/userfindetails';
  ...
  EditObj(Obj: any): Observable<any> {
    const headerDict = {
      'Content-Type': 'application/json',
      'x-token': ''+ localStorage.getItem('token')+''
    }
 ...
 getSecDispatcher(params: any): Promise<any>{
