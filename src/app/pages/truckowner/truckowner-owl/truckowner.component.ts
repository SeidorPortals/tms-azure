...
@Component({
  selector: 'app-truckowner',
  templateUrl: './truckowner.component.html',
  styleUrls: ['./truckowner.component.css']
})
export class TruckownerComponent implements OnInit {
  ObjList = [];
  ObjListOriginal = [];
 ...
 constructor(private ObjServiceService: TruckownerService,  private toastEvoke: ToastEvokeService, ) { }
      ...
      resp => {
        if(Array.isArray(resp)){
          if(resp.length > 0){
            ...
            localStorage.truckowner = JSON.stringify(resp);
          }else{
            this.ObjList = [];
