...
@Component({
  selector: 'app-rooftype-owl',
  templateUrl: './rooftype-owl.component.html',
  styleUrls: ['./rooftype-owl.component.css']
})
export class RooftypeOwlComponent implements OnInit {
 ...
 constructor(
   private ObjServiceService: RoofTypeService, private toastEvoke: ToastEvokeService
 ) { }
     ...
     resp => {
      if(Array.isArray(resp)){
        if(resp.length > 0){
          ...
          localStorage.roofType = JSON.stringify(resp);
        }else{
          this.ObjList = [];
